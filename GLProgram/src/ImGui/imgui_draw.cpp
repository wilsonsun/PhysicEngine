...
// Scan and shade backward from the end of given verticesGraphic. Assume verticesGraphic are text only (= vert_start..vert_end going left to right) so we can break as soon as we are out the gradient bounds.
...
void ImFont::RenderText(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width, bool cpu_fine_clip) const
{
    ...
    // Reserve verticesGraphic for remaining worse case (over-reserving is useful and easily amortized)
    ...
    // Give back unused verticesGraphic
